...
async function handleAdminApi(request, env, url, ctx) {
  ...
  if (url.pathname === "/api/admin/leads" && request.method === "GET") {
    const result=await env.DB.prepare("SELECT id,marketplace,count,product,style,deadline,contact,comment,status,client_decision,client_comment,client_decision_at,source,medium,campaign,content,referrer,landing,created_at FROM leads ORDER BY datetime(created_at) DESC,id DESC LIMIT 300").all();
  ...
  if(leadMatch && request.method==="PATCH"){
    ...
    if(body.status==="review"){
      await env.DB.prepare("UPDATE leads SET status=?,client_decision=NULL,client_decision_at=NULL WHERE id=?").bind(body.status,Number(leadMatch[1])).run();
    }else{
  ...
  async fetch(request, env, ctx) {
    ...
    if ((url.pathname === "/" || url.pathname === "/index.html") && response.headers.get("content-type")?.includes("text/html")) {
      return new HTMLRewriter()
        ...
        .on("head", { element(element) { element.append(`<meta name="description" content="AuraFX — дизайн карточек товаров для маркетплейсов. Портфолио, кейсы и бесплатный мини-разбор вашей карточки."><meta name="theme-color" content="#0b0612"><meta name="color-scheme" content="dark"><meta property="og:site_name" content="AuraFX"><link rel="icon" type="image/png" sizes="48x48" href="${AURAFX_FAVICON_DATA_URL}"><meta property="og:title" content="AuraFX — дизайн карточек товаров"><meta property="og:description" content="Дизайн карточек товаров: портфолио, кейсы и бесплатный мини-разбор карточки."><meta property="og:type" content="website"><meta property="og:url" content="https://aurafx-site.pages.dev/">`, { html: true }); } })
