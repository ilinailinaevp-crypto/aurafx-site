...
<style>
  ...
  @media(max-width:760px){
    ...
    #afx-function-nav.is-collapsed{top:70px;left:8px;right:auto}
  }
...
(()=>{
  ...
  const labels={bot:'Помощник',open:'Ждём оператора',waiting_client:'Ждём тебя',resolved:'Решён'},cats={technical:'Техническая проблема',order:'Вопрос по заказу',payment:'Оплата',general:'Другое'};
  const esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
...
function renderMessages(d){const t=d.thread,m=d.messages||[];$('#chatTitle').textContent=t.subject;$('#chatMeta').textContent=(cats[t.category]||t.category)+' · '+(labels[t.status]||t.status)+' · #'+t.id;$('#resolveBtn').classList.toggle('hidden',t.status==='resolved');$('#handoffBtn').classList.toggle('hidden',t.status!=='bot');$('#composer').classList.remove('hidden');const box=$('#messages');if(!m.length)box.innerHTML='<div class="empty">Сообщений пока нет.</div>';else box.innerHTML=m.map(x=>'<div class="bubble '+(x.sender_role==='client'?'client':'admin')+'">'+esc(x.body)+'<small>'+(x.sender_role==='bot'?'AuraFX Помощник':x.sender_role==='admin'?'AuraFX Support':'Вы')+' · '+esc(fmt(x.created_at))+'</small></div>').join('');if(m.length!==lastMessageCount){box.scrollTop=box.scrollHeight;lastMessageCount=m.length}}
...
(()=>{const $=s=>document.querySelector(s),esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));let threads=[],activeId=0,filter='all',lastCount=0;const labels={bot:'Помощник',open:'Нужен оператор',waiting_client:'Ждём клиента',resolved:'Решён'},cats={technical:'Техническая проблема',order:'Вопрос по заказу',payment:'Оплата',general:'Другое'};function fmt(v){try{return new Date(String(v)+'Z').toLocaleString('ru-RU',{day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'})}catch{return ''}}async function api(url,opt){const r=await fetch(url,{headers:{accept:'application/json','content-type':'application/json'},...opt});let d={};try{d=await r.json()}catch{}if(r.status===401){$('#loginBox').classList.remove('hidden');$('#app').classList.add('hidden');throw new Error('AUTH')}if(!r.ok)throw new Error(d.error||'Ошибка');return d}
...
function supportBotAnswer(raw) {
  const q=String(raw||"").toLowerCase().replace(/ё/g,"е");
  if (/(оператор|админ|человек|живой|менеджер|сотрудник|позови|позвать|связаться)/.test(q)) return null;
  if (/(мой|моего|моя|мою|мне|у меня|личн|не приш|пропал|исчез|ошибк|проблем)/.test(q)) return null;
  if (/(цен|стоим|тариф|рубл|сколько стоит)/.test(q)) return "Тарифы AuraFX: 1 карточка — 150 ₽, 3 — 450 ₽, 5 — 750 ₽, 10 — 1 500 ₽. Сложная ретушь, 3D и дополнительные форматы оцениваются после просмотра задачи. Срочный срок — доплата 50%, если есть свободный слот.";
  if (/(сроч|срок|быстр|когда готов|сколько дней)/.test(q)) return "Обычный ориентир — 2–4 дня после получения ТЗ и материалов. Срочный заказ возможен от 24 часов с доплатой 50%; точное время AuraFX подтвердит лично до начала работы.";
  if (/(правк|измен|передел|согласован)/.test(q)) return "В стоимость включены два круга правок по согласованному ТЗ. По готовому макету в личном кабинете можно принять результат или отправить конкретные замечания.";
  if (/(заказ|заявк|бриф|оформ|начать проект)/.test(q)) return "Оставь заявку в приложении в разделе «Заявка» или на сайте aurafx-design.ru. Укажи товар, площадку, число карточек, срок и контакт. Всё поступит в общую систему AuraFX; после входа через Telegram заказ появится в кабинете.";
  if (/(отзыв|оценк|комментарий клиент)/.test(q)) return "Отзывы можно посмотреть и отправить в разделе «Отзывы». Новый отзыв проходит модерацию перед публикацией.";
  if (/(кейс|портфолио|пример работ|работы)/.test(q)) return "В разделе «Кейсы» есть 19 демонстрационных концептов. Открой карточку и листай пять слайдов, чтобы увидеть полную подачу товара.";
  if (/(вход|аккаунт|телеграм|telegram|кабинет|код)/.test(q)) return "В кабинете нажми «Войти через Telegram» и подтверди вход через @AuraFXAuthBot. Заявки и обращения связаны с этим аккаунтом. Если вход не получается, напиши «Позвать оператора».";
  if (/(оплат|сбп|картой|счет|чек)/.test(q)) return "Условия оплаты и доступные способы AuraFX подтвердит лично для твоего заказа. Напиши «Позвать оператора», чтобы обсудить оплату без догадок.";
  if (/(формат|размер|wildberries|ozon|avito|маркетплейс)/.test(q)) return "AuraFX готовит файлы под выбранную площадку. Укажи в заявке Wildberries, Ozon, Avito или другую площадку и нужный формат — детали согласуют перед работой.";
  return null;
}
async function supportBotProcess(env,ctx,request,id,subject,text) {
  const answer=supportBotAnswer(text);
  if (answer) {
    await env.DB.prepare("INSERT INTO support_messages (thread_id,sender_role,body) VALUES (?,'bot',?)").bind(id,answer+"\n\nЕсли нужен человек, напиши «Позвать оператора».").run();
    await env.DB.prepare("UPDATE support_threads SET status='bot',updated_at=datetime('now') WHERE id=?").bind(id).run();
    return "bot";
  }
  await env.DB.prepare("INSERT INTO support_messages (thread_id,sender_role,body) VALUES (?,'bot',?)").bind(id,"Передаю вопрос человеку. Администратор увидит всю нашу переписку и ответит здесь.").run();
  await env.DB.prepare("UPDATE support_threads SET status='open',updated_at=datetime('now') WHERE id=?").bind(id).run();
  const notify="🙋 AuraFX Support · нужен оператор в чате #"+id+"\n"+String(subject||"")+"\n\n"+String(text||"").slice(0,700)+"\n\nОткрыть: "+new URL("/admin/support",request.url).toString();
...
async function handleAccountApi(request,env,url,ctx){
  ...
  if(url.pathname==="/api/account/support"&&request.method==="POST"){
    ...
    const ins=await env.DB.prepare("INSERT INTO support_threads (telegram_id,subject,category,status,client_last_read_at) VALUES (?,?,?,'bot',datetime('now'))").bind(user.id,subject,category).run();
    ...
    const status=await supportBotProcess(env,ctx,request,id,subject,message);
  ...
  if(supportMessage&&request.method==="POST"){
    if(!sameOrigin(request))return json({error:"Запрос отклонён."},403);
    const id=Number(supportMessage[1]);const thread=await env.DB.prepare("SELECT id,telegram_id,subject,status FROM support_threads WHERE id=? AND telegram_id=? LIMIT 1").bind(id,user.id).first();
    if(!thread)return json({error:"Обращение не найдено."},404);
    ...
    await env.DB.prepare("UPDATE support_threads SET updated_at=datetime('now'),client_last_read_at=datetime('now') WHERE id=? AND telegram_id=?").bind(id,user.id).run();
    if(thread.status==="bot"||thread.status==="resolved")return json({ok:true,status:await supportBotProcess(env,ctx,request,id,thread.subject,text)});
    await env.DB.prepare("UPDATE support_threads SET status='open' WHERE id=?").bind(id).run();
    const notify="💬 AuraFX Support · сообщение в #"+id+"\n"+String(thread.subject||"")+"\n\n"+text.slice(0,900)+"\n\nОткрыть: "+new URL("/admin/support",request.url).toString();
    if(ctx&&typeof ctx.waitUntil==="function")ctx.waitUntil(sendTelegram(env,notify).catch(()=>({sent:false})));
    return json({ok:true,status:"open"});
  }
  const supportHandoff=url.pathname.match(/^\/api\/account\/support\/(\d+)\/handoff$/);
  if(supportHandoff&&request.method==="POST"){
    if(!sameOrigin(request))return json({error:"Запрос отклонён."},403);
    const id=Number(supportHandoff[1]);const t=await env.DB.prepare("SELECT id,subject,status FROM support_threads WHERE id=? AND telegram_id=? LIMIT 1").bind(id,user.id).first();
    if(!t)return json({error:"Обращение не найдено."},404);
    if(t.status==="open"||t.status==="waiting_client")return json({ok:true,status:t.status});
    const status=await supportBotProcess(env,ctx,request,id,t.subject,"Позвать оператора");
...
async function handleAdminApi(request, env, url, ctx) {
  ...
  if(url.pathname==="/api/admin/support"&&request.method==="GET"){
    const r=await env.DB.prepare(`SELECT t.id,t.telegram_id,t.subject,t.category,t.status,t.created_at,t.updated_at,u.name,u.username,
      ...
      ORDER BY CASE t.status WHEN 'open' THEN 0 WHEN 'waiting_client' THEN 1 WHEN 'bot' THEN 2 ELSE 3 END,datetime(t.updated_at) DESC,t.id DESC LIMIT 200`).all();
    return json({threads:r.results||[]});
  }
