    ...
    <div class="afx-r-grid">
      <div class="afx-r-panel">
        ...
        <form class="afx-r-form" id="afx-review-form">
          ...
          <button class="afx-r-submit" type="submit">Опубликовать отзыв →</button>
...
(()=>{
  ...
  form.addEventListener('submit',async e=>{
    e.preventDefault();status.className='afx-r-status';status.textContent='Проверяем и публикуем…';const btn=form.querySelector('button[type="submit"]');btn.disabled=true;
    const fd=new FormData(form);const payload={name:fd.get('name'),text:fd.get('text'),company:fd.get('company'),rating};
    try{const res=await fetch('/api/reviews',{method:'POST',headers:{'content-type':'application/json','accept':'application/json'},body:JSON.stringify(payload)});const data=await res.json();if(!res.ok)throw new Error(data.error||'Не удалось отправить отзыв');status.className='afx-r-status ok';status.textContent='Готово — отзыв опубликован ✨';form.reset();rating=5;paint();await load()}
...
function moderationReason(name, text) {
  ...
  if (badWords.test(value)) return "Похоже, в отзыве есть мат. Перефразируй его без ругани 🙂";
  if (/(?:https?:\/\/|www\.|t\.me\/|@\w{4,})/iu.test(value)) return "Ссылки и рекламные контакты в отзывах не публикуются.";
  if (/(.)\1{7,}/u.test(value)) return "Похоже на спам. Напиши отзыв обычным текстом.";
  const letters = value.replace(/[^а-яa-z]/giu, "");
  if (letters.length > 20 && new Set(letters).size < 4) return "Похоже на спам. Напиши отзыв обычным текстом.";
  return null;
}
...
async function handleApi(request, env) {
  ...
  const moderation = moderationReason(name, text);
  if (moderation) return json({ error: moderation }, 400);
  ...
  await env.DB.prepare("INSERT INTO reviews (name, rating, text, status, ip_hash) VALUES (?, ?, ?, 'approved', ?)").bind(name, rating, text, ipHash).run();
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/api/reviews") return handleApi(request, env);
