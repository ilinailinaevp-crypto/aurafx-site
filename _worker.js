...
self.addEventListener('fetch',e=>{if(e.request.method!=='GET')return;const u=new URL(e.request.url);if(u.origin!==location.origin)return;if(/^\/(?:api|auth|admin|account|referral|support)(?:\/|$)/.test(u.pathname))return;if(e.request.mode==='navigate'){e.respondWith(fetch(e.request).then(r=>{const copy=r.clone();caches.open(CACHE).then(c=>c.put('/',copy)).catch(()=>{});return r}).catch(()=>caches.match('/')));return}e.respondWith(caches.match(e.request).then(hit=>hit||fetch(e.request).then(r=>{if(r.ok)caches.open(CACHE).then(c=>c.put(e.request,r.clone())).catch(()=>{});return r}))) });`;
...
<style>
#afx-support-fab{position:fixed;right:16px;bottom:18px;z-index:9996;display:inline-flex;align-items:center;gap:8px;padding:12px 15px;border:1px solid rgba(182,105,255,.26);border-radius:999px;background:linear-gradient(135deg,rgba(28,13,47,.96),rgba(72,24,117,.96));color:#fff;text-decoration:none;font:850 12px/1 system-ui,-apple-system,Segoe UI,sans-serif;box-shadow:0 16px 45px rgba(0,0,0,.34),0 0 30px rgba(153,66,255,.14);backdrop-filter:blur(16px);transition:.2s transform,.2s border-color}
#afx-support-fab:hover{transform:translateY(-2px);border-color:rgba(121,231,255,.45)}#afx-support-fab i{font-style:normal;font-size:15px}
@media(max-width:620px){#afx-support-fab{right:10px;bottom:10px;padding:11px 13px}}
</style><a id="afx-support-fab" href="/support" aria-label="Открыть техподдержку AuraFX"><i>💬</i><span>Техподдержка</span></a>`;
...
@media(max-width:780px){.shell{grid-template-columns:1fr;min-height:auto}.side{max-height:330px}.chat{min-height:620px}.bubble{max-width:88%}.top{align-items:flex-start}.top>div:last-child{display:flex;gap:7px;flex-wrap:wrap;justify-content:flex-end}}
...
async function handleAuthApi(request, env, url) {
  if (url.pathname === "/api/auth/telegram/webhook" && request.method === "POST") {
    ...
    if (plainStart || ordersCmd || helpCmd || refCmd || supportCmd) {
      await ensureDb(env);
      ...
      if (supportCmd) {
        ...
        try{await telegramLoginApi(env,"sendMessage",{chat_id:chatId,text:"💬 Техподдержка AuraFX работает прямо на сайте. Там сохраняется история переписки и можно продолжить диалог в любое время.",reply_markup:{inline_keyboard:[[{text:"Открыть поддержку",url:supportUrl}],[{text:"Личный кабинет",url:accountUrl}]]}})}catch{}
        return json({ok:true});
      }
      if (refCmd) {
        ...
        try{await telegramLoginApi(env,"sendMessage",{chat_id:chatId,text:"💜 Твоя реферальная ссылка AuraFX:\n"+link+"\n\nЗа первого завершённого заказа каждого приглашённого клиента — 1 карточка бесплатно.\n\nПолучено бонусов: "+Number(rs?.earned||0)+" · доступно: "+Number(rs?.available||0),reply_markup:{inline_keyboard:[[{text:"Реферальный центр",url:center}],[{text:"Личный кабинет",url:accountUrl}]]}})}catch{}
        return json({ok:true});
      }
      const welcome=plainStart?"Привет! Это клиентский центр AuraFX 💜\n\nЗдесь можно быстро проверить заказы, открыть поддержку и получить реферальную ссылку.":"Команды AuraFX:\n/orders — мои заявки\n/ref — моя реферальная ссылка\n/support — техподдержка\n/help — помощь";
